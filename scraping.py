...
for i in range(16597, len(vgsales['Name'])):
    #df = pd.concat([df, scrape_single(driver, vgsales['Name'][i])], ignore_index=True)
    df = scrape_single(driver, vgsales['Name'][i])

    if df is not None:
        with open('esrb_ratings.csv', 'a') as f:
